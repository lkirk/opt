...
int subcommand_1_main(int argc, char **argv, opt_t *state) {
    int ret;
    opt_arg_t *arg;
    opt_config_t cli_config = {
        .parse_long = 1,
        .add_help = 1,
        .subcommand_required = 0,
        .description = "Descriptive description of subcommand 1",
        .subcommands = {NULL},
        .args = {ARG('o', "o-opt", OPT_REQUIRED | OPT_HAS_ARG,
    ...
    char *o = "";
    if ((opt_init(state, &cli_config, 1)) == -1)
        return -1;
    while ((ret = opt_parse(argc, argv, state, &cli_config)) > 0) {
        switch (ret) {
        case OPT_OK:
            ...
            case 'o':
                ...
                break;
            case 'h':
                opt_usage(argv[0], state, subc_name, &cli_config);
                exit(1);
            }
            break;
        }
    ...
    if (ret < 0) {
        opt_print_error(ret, state, argv);
        exit(1);
    }
...
int subcommand_2_main(int argc, char **argv, opt_t *state) {
    int ret;
    opt_arg_t *arg;
    opt_config_t cli_config = {
        .parse_long = 1,
...
int subcommand_3_main(int argc, char **argv, opt_t *state) {
    int ret;
    opt_arg_t *arg;
    opt_config_t cli_config = {
        .parse_long = 1,
        ...
        .args = {
            ...
            POSITIONAL_ARG("file", OPT_POSITIONAL_MULTIPLE, "input file(s)"),
            NULL}};
    // TODO: chain multiple subc names in nested subcs... up to user?
    char *subc_name = state->optarg;
...
int main(int argc, char **argv) {
    ...
    opt_config_t cli_config = {
        ...
        .args = {
            ARG('a', "a-opt", OPT_REQUIRED | OPT_HAS_ARG,
                "This is a required option, providing the A "
                "parameter."),
            ARG('b', "b-flag", OPT_REQUIRED,
    ...
    if ((opt_init(&state, &cli_config, 0)) == -1)
        return -1;
    while ((ret = opt_parse(argc, argv, &state, &cli_config)) > 0) {
        arg = cli_config.args[state.argind];
        switch (ret) {
        case OPT_OK: {
            ...
            case 'd':
                ...
                break;
            case 'h':
                opt_usage(argv[0], &state, NULL, &cli_config);
                exit(1);
            default:
